...
typedef enum {
    TOKEN_PROGRAMA, TOKEN_IDENTIFICADOR, TOKEN_INTEIRO, TOKEN_BOOLEANO,
    TOKEN_INICIO, TOKEN_FIM, TOKEN_LEIA, TOKEN_ESCREVA, TOKEN_SE,
    TOKEN_SENAOSE, TOKEN_PARA, TOKEN_AJUSTAR, TOKEN_ATE, TOKEN_DE,
...
TipoToken verificar_palavra_reservada(const char *lexema) {
    ...
    if (strcmp(lexema, "boolean") == 0) return TOKEN_BOOLEANO;
    ...
    if (strcmp(lexema, "if") == 0) return TOKEN_SE;
    if (strcmp(lexema, "elif") == 0) return TOKEN_SENAOSE;
    if (strcmp(lexema, "for") == 0) return TOKEN_PARA;
    if (strcmp(lexema, "set") == 0) return TOKEN_AJUSTAR;
